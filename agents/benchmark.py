...
def run_experiment(algo_name: str, total_steps: int = 600, eval_interval: int = 100):
    ...
    for step in range(1, total_steps + 1):
        ...
        if step % eval_interval == 0:
            ...
            print(
                (
                    f"[{algo_name}] Step {step}/{total_steps} | Eval Reward: "
                    f"{eval_metrics['total_reward']:.2f} | SU Thr: {eval_metrics['throughput_s']:.3f} | "
    ...
    print(
        f"[{algo_name}] Finished in {train_time:.2f}s | Avg Inference Time: "
...
def generate_plots(results: dict) -> None:
    ...
    inf_times = [results[n]["inf_time"] * 1000.0 for n in algos]

    x = np.arange(len(algos))
    width = 0.35

    ax1.bar(x - width / 2, train_times, width, label="Training Time (s)", color="#9b59b6")
    ax2.bar(x + width / 2, inf_times, width, label="Inference Time (ms)", color="#f1c40f")
...
def main() -> None:
    ...
    print(
        "| Algorithm | Training Time (s) | Avg Inference Time (ms) | Final Eval SU Rate | Final PU Outage |"
    )
    print("|---|---|---|---|---|")
    for name in results:
        h = results[name]["history"]
        final_su_thr = h["throughput_s"][-1] if h["throughput_s"] else float("nan")
        final_pu_out = h["outage"][-1] if h["outage"] else float("nan")
        print(
            (
                f"| {name} | {results[name]['train_time']:.2f}s | "
                f"{results[name]['inf_time']*1000.0:.3f}ms | {final_su_thr:.4f} | {final_pu_out:.4f} |"
            )
        )
    print("======================================================")
