"""
Benchmark tool to train, evaluate, and compare TD3, CAMO-TD3, and OVERLAY_CAMO_TD3.
Generates metrics graphs, measures training/inference speeds, and reports summary statistics.
"""
...
def run_experiment(algo_name: str, total_steps: int = 2000 * 300, eval_interval: int = 3000):
    ...
    for step in range(1, total_steps + 1):
        ...
        if step % eval_interval == 0:
            ...
            print(f"[{algo_name}] Step {step}/{total_steps} | Eval Reward: {eval_metrics['total_reward']:.2f} | SU Thr: {eval_metrics['throughput_s']:.3f} | PU Outage: {eval_metrics['outage']:.4f}")
    ...
    print(f"[{algo_name}] Finished in {train_time:.2f}s | Avg Inference Time: {avg_inf_time*1000.0:.3f}ms")
...
def generate_plots(results):
    ...
    plt.figure()
    for name in algos:
        plt.plot(results[name]["history"]["steps"], results[name]["history"]["throughput_s"], label=name, color=colors[name], marker='o')
    ...
    plt.figure()
    for name in algos:
        plt.plot(results[name]["history"]["steps"], results[name]["history"]["ber"], label=name, color=colors[name], marker='x')
    plt.xlabel("Training Steps")
    ...
    plt.figure()
    for name in algos:
        plt.plot(results[name]["history"]["steps"], results[name]["history"]["outage"], label=name, color=colors[name], marker='s')
    plt.xlabel("Training Steps")
    ...
    plt.figure()
    for name in algos:
        plt.plot(results[name]["history"]["steps"], results[name]["history"]["rewards"], label=name, color=colors[name], marker='d')
    plt.xlabel("Training Steps")
...
def main():
    ...
    print("| Algorithm | Training Time (s) | Avg Inference Time (ms) | Final Eval SU Rate | Final PU Outage |")
    print("|---|---|---|---|---|")
    for name in results:
        h = results[name]["history"]
        final_su_thr = h["throughput_s"][-1]
        final_pu_out = h["outage"][-1]
        print(f"| {name} | {results[name]['train_time']:.2f}s | {results[name]['inf_time']*1000.0:.3f}ms | {final_su_thr:.4f} | {final_pu_out:.4f} |")
    print("======================================================")
