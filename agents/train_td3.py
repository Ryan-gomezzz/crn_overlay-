...
class TD3Agent:
    ...
    def __init__(self, config: dict, device: str = "cpu"):
        ...
        if self.algorithm_name == "CAMO_TD3":
            ...
            # Store in log scale to enforce positive lambdas via exp or softplus
    ...
    def select_action(self, obs: np.ndarray, info: dict, explore: bool = True) -> np.ndarray:
        ...
        If CAMO-TD3 or OVERLAY_CAMO_TD3, uses history sequences and GRU belief state.
        """
        if self.algorithm_name == "OVERLAY_CAMO_TD3":
            ...
            if explore:
                ...
                # Compute gradient of constraint Q1 values with respect to action
                belief_tensor = belief.clone().detach().requires_grad_(True)
                action_tensor = torch.as_tensor(action, dtype=torch.float32, device=self.device).unsqueeze(0).requires_grad_(True)
    ...
    def train(self, writer: SummaryWriter) -> dict:
        ...
        if self.algorithm_name == "OVERLAY_CAMO_TD3":
            ...
            # Detach belief for critic updates to prevent multiple backward passes
            ...
            # 4. Update Adaptive Lagrangian multipliers (gradient ascent)
            ...
            # Compute actual constraint violations compared to limit thresholds
            # We use Q1 estimation at the sampled state and action
            ...
            if self.total_it % self.policy_delay == 0:
                ...
                # Update Encoder weights to maximize Q values representation
                # We backpropagate w.r.t the encoder parameters
