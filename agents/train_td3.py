...
from agents.models import GRUBeliefEncoder, CAMO_Actor, TwinCritics, TD3_Actor
...
class TD3Agent:
    ...
    Handles online interaction, action selection, optimization, logging, and checkpointing.
    """
    ...
    def __init__(self, config: dict, device: str = "cpu"):
        ...
        self.algorithm_name = name_map.get(
        ...
        if self.algorithm_name == "CAMO_TD3":
            ...
            # Actor Network
            self.actor = CAMO_Actor(
                belief_dim=64, action_dim=self.action_dim
            ).to(self.device)
            self.actor_target = CAMO_Actor(
                belief_dim=64, action_dim=self.action_dim
            ).to(self.device)
            ...
            # Critics: 3 Separate Twin Critic Pairs
            # (Throughput, Interference, Energy)
            self.critic_thr = TwinCritics(
                state_dim=64, action_dim=self.action_dim
            ).to(self.device)
            self.critic_thr_target = TwinCritics(
                state_dim=64, action_dim=self.action_dim
            ).to(self.device)
            self.critic_thr_target.load_state_dict(
                self.critic_thr.state_dict()
            )
            self.critic_thr_optimizer = optim.Adam(
                self.critic_thr.parameters(), lr=self.lr_critic
            )

            self.critic_inf = TwinCritics(
                state_dim=64, action_dim=self.action_dim
            ).to(self.device)
            self.critic_inf_target = TwinCritics(
                state_dim=64, action_dim=self.action_dim
            ).to(self.device)
            self.critic_inf_target.load_state_dict(
                self.critic_inf.state_dict()
            )
            self.critic_inf_optimizer = optim.Adam(
                self.critic_inf.parameters(), lr=self.lr_critic
            )

            self.critic_nrg = TwinCritics(
                state_dim=64, action_dim=self.action_dim
            ).to(self.device)
            self.critic_nrg_target = TwinCritics(
                state_dim=64, action_dim=self.action_dim
            ).to(self.device)
            self.critic_nrg_target.load_state_dict(
                self.critic_nrg.state_dict()
            )
            self.critic_nrg_optimizer = optim.Adam(
                self.critic_nrg.parameters(), lr=self.lr_critic
            )
            ...
            # Recurrent GRU Belief Encoders
            # (input_dim = 8 for obs, act, dec, out)
            self.encoder = GRUBeliefEncoder(
                self.obs_dim,
                self.action_dim,
                hidden_dim=64,
            ...
            self.critic_qos = TwinCritics(
                state_dim=64, action_dim=self.action_dim
            ).to(self.device)
            self.critic_qos_target = TwinCritics(
                state_dim=64, action_dim=self.action_dim
            ).to(self.device)
            self.critic_qos_target.load_state_dict(
                self.critic_qos.state_dict()
            )
            self.critic_qos_optimizer = optim.Adam(
                self.critic_qos.parameters(), lr=self.lr_critic
            )
    ...
    def select_action(
        ...
        if self.algorithm_name == "OVERLAY_CAMO_TD3":
            ...
            history = np.concatenate(
                [obs_history, act_history, dec_history, out_history],
                axis=-1,
            )
            ...
            if explore:
                ...
                grad_qos = torch.autograd.grad(
                    q_qos.sum(), action_tensor, retain_graph=True
                )[0].cpu().data.numpy().flatten()
                grad_nrg = torch.autograd.grad(
                    q_nrg.sum(), action_tensor
                )[0].cpu().data.numpy().flatten()

                safety_bias = self.eta_explore * (
                    self.lambda_qos * grad_qos - self.lambda_nrg * grad_nrg
                )
            ...
            if explore:
                ...
                grad_inf = torch.autograd.grad(
                    q_inf.sum(), action_tensor, retain_graph=True
                )[0].cpu().data.numpy().flatten()
                grad_nrg = torch.autograd.grad(
                    q_nrg.sum(), action_tensor
                )[0].cpu().data.numpy().flatten()

                # Directional explore vector
                # - eta * (lambda_inf * grad_inf + lambda_nrg * grad_nrg)
                # Moving in negative direction decreases violation risk
                safety_bias = -self.eta_explore * (
                    self.lambda_inf * grad_inf + self.lambda_nrg * grad_nrg
                )
    ...
    def train(self, writer: SummaryWriter) -> dict:
        ...
        if self.algorithm_name == "OVERLAY_CAMO_TD3":
            ...
            if self.total_it % self.policy_delay == 0:
                ...
                q_thr_pred = self.critic_thr.Q1(
                    detached_belief, actions_pred
                )
                q_qos_pred = self.critic_qos.Q1(
                    detached_belief, actions_pred
                )
                q_nrg_pred = self.critic_nrg.Q1(
                    detached_belief, actions_pred
                )
                ...
                actor_loss = (
                    -(q_thr_pred - penalty_qos - penalty_nrg).mean()
                )
                ...
                actor_loss_recurrent = -(
                    self.critic_thr.Q1(
                        encoder_belief, actions_pred.detach()
                    )
                ).mean()
                ...
                for param, target_param in zip(
                    self.critic_thr.parameters(),
                    self.critic_thr_target.parameters(),
                ):
                    update = (
                        self.tau * param.data
                        + (1 - self.tau) * target_param.data
                ...
                for param, target_param in zip(
                    self.critic_qos.parameters(),
                    self.critic_qos_target.parameters(),
                ):
                    update = (
                        self.tau * param.data
                        + (1 - self.tau) * target_param.data
                ...
                for param, target_param in zip(
                    self.critic_nrg.parameters(),
                    self.critic_nrg_target.parameters(),
                ):
                    update = (
                        self.tau * param.data
                        + (1 - self.tau) * target_param.data
            ...
            # Log metrics
            writer.add_scalar(
                "Loss/Critic_Throughput",
                loss_critic_thr.item(),
                self.total_it,
            )
            writer.add_scalar(
                "Loss/Critic_QoS", loss_critic_qos.item(), self.total_it
            )
            writer.add_scalar(
                "Loss/Critic_Energy", loss_critic_nrg.item(), self.total_it
            )
            writer.add_scalar(
                "Lagrangian/Lambda_QoS", self.lambda_qos, self.total_it
            )
            writer.add_scalar(
                "Lagrangian/Lambda_Energy", self.lambda_nrg, self.total_it
            )
            writer.add_scalar(
                "Lagrangian/Violation_QoS_bps_Hz",
                violation_qos.mean().item(),
                self.total_it,
            )
            writer.add_scalar(
                "Lagrangian/Violation_Energy_Watts",
                violation_nrg.mean().item(),
                self.total_it,
            )
            ...
            with torch.no_grad():
                ...
                # Select target action with smoothing noise
                noise = (
                    torch.randn_like(next_act_seq[:, -1, :])
                    * self.policy_noise
                ).clamp(-self.noise_clip, self.noise_clip)
                next_action = (
                    self.actor_target(next_belief) + noise
            ...
            # 4. Update Adaptive Lagrangian multiplier
            # Loss = - lambda_inf * (Q_inf - limit)
            #        - lambda_nrg * (Q_nrg - limit)
            ...
            # 5. Delayed Policy & Encoder Updates
            if self.total_it % self.policy_delay == 0:
                # Update Actor to maximize Throughput
                detached_belief = belief.detach()
                actions_pred = self.actor(detached_belief)
                ...
                actor_loss = (
                    -(q_thr_pred - penalty_inf - penalty_nrg).mean()
                )
                ...
                eq_thr = self.critic_thr.Q1(
                    encoder_belief.detach(), act_seq[:, -1, :]
                )
                eq_inf = self.critic_inf.Q1(
                    encoder_belief.detach(), act_seq[:, -1, :]
                )
                eq_nrg = self.critic_nrg.Q1(
                    encoder_belief.detach(), act_seq[:, -1, :]
                )
                ...
                for param, target_param in zip(
                    self.critic_inf.parameters(),
                    self.critic_inf_target.parameters(),
                ):
                    update = (
                        self.tau * param.data
                        + (1 - self.tau) * target_param.data
            ...
            writer.add_scalar(
                "Loss/Critic_Interference",
                loss_critic_inf.item(),
                self.total_it,
            )
            writer.add_scalar(
                "Loss/Critic_Energy", loss_critic_nrg.item(), self.total_it
            ...
            writer.add_scalar(
                "Lagrangian/Lambda_Interference",
                self.lambda_inf,
                self.total_it,
            )
            writer.add_scalar(
            ...
            writer.add_scalar(
                "Lagrangian/Violation_Interference_Watts",
                violation_inf.mean().item(),
                self.total_it,
            )
            writer.add_scalar(
                "Lagrangian/Violation_Energy_Watts",
            ...
            obs, action, reward, next_obs, done = (
                self.replay_buffer.sample_standard(self.batch_size)
            )
            ...
            checkpoint.update(
                {
                    ...
                    "encoder_target_state_dict": (
                        self.encoder_target.state_dict()
                    ),
                    "encoder_optimizer_state_dict": (
                        self.encoder_optimizer.state_dict()
                    ),
                    "critic_thr_state_dict": self.critic_thr.state_dict(),
                    "critic_thr_target_state_dict": (
                        self.critic_thr_target.state_dict()
                    ),
                    "critic_thr_optimizer_state_dict": (
                        self.critic_thr_optimizer.state_dict()
                    ),
                    "critic_inf_state_dict": self.critic_inf.state_dict(),
                    "critic_inf_target_state_dict": (
                        self.critic_inf_target.state_dict()
                    ),
                    "critic_inf_optimizer_state_dict": (
                        self.critic_inf_optimizer.state_dict()
                    ),
                    "critic_nrg_state_dict": self.critic_nrg.state_dict(),
                    "critic_nrg_target_state_dict": (
                        self.critic_nrg_target.state_dict()
                    ),
                    "critic_nrg_optimizer_state_dict": (
                        self.critic_nrg_optimizer.state_dict()
                    ),
                    "alpha_inf": self.alpha_inf.item(),
                    "alpha_nrg": self.alpha_nrg.item(),
                    "lambda_optimizer_state_dict": (
            ...
            checkpoint.update(
                {
                    ...
                    "critic_qos_target_state_dict": (
                        self.critic_qos_target.state_dict()
                    ),
                    "critic_qos_optimizer_state_dict": (
                        self.critic_qos_optimizer.state_dict()
                    ),
                    "critic_nrg_state_dict": self.critic_nrg.state_dict(),
                    "critic_nrg_target_state_dict": (
                        self.critic_nrg_target.state_dict()
    ...
    def load(self, filepath: str):
        ...
        if self.algorithm_name == "CAMO_TD3":
            ...
            self.encoder_target.load_state_dict(
                checkpoint["encoder_target_state_dict"]
            )
            self.encoder_optimizer.load_state_dict(
                checkpoint["encoder_optimizer_state_dict"]
            )
            self.critic_thr.load_state_dict(
                checkpoint["critic_thr_state_dict"]
            )
            self.critic_thr_target.load_state_dict(
                checkpoint["critic_thr_target_state_dict"]
            )
            self.critic_thr_optimizer.load_state_dict(
                checkpoint["critic_thr_optimizer_state_dict"]
            )
            self.critic_inf.load_state_dict(
                checkpoint["critic_inf_state_dict"]
            )
            self.critic_inf_target.load_state_dict(
                checkpoint["critic_inf_target_state_dict"]
            )
            self.critic_inf_optimizer.load_state_dict(
                checkpoint["critic_inf_optimizer_state_dict"]
            )
            self.critic_nrg.load_state_dict(
                checkpoint["critic_nrg_state_dict"]
            )
            self.critic_nrg_target.load_state_dict(
                checkpoint["critic_nrg_target_state_dict"]
            )
            self.critic_nrg_optimizer.load_state_dict(
                checkpoint["critic_nrg_optimizer_state_dict"]
            )
            self.alpha_inf.data = torch.tensor(
                checkpoint["alpha_inf"],
                dtype=torch.float32,
                device=self.device,
            )
            self.alpha_nrg.data = torch.tensor(
                checkpoint["alpha_nrg"],
                dtype=torch.float32,
                device=self.device,
            )
            ...
            self.critic_qos.load_state_dict(
                checkpoint["critic_qos_state_dict"]
            )
            self.critic_qos_target.load_state_dict(
                checkpoint["critic_qos_target_state_dict"]
            )
            self.critic_qos_optimizer.load_state_dict(
                checkpoint["critic_qos_optimizer_state_dict"]
            )
            self.critic_nrg.load_state_dict(
                checkpoint["critic_nrg_state_dict"]
            )
            ...
            self.alpha_qos.data = torch.tensor(
                checkpoint["alpha_qos"],
                dtype=torch.float32,
                device=self.device,
            )
            self.alpha_nrg.data = torch.tensor(
